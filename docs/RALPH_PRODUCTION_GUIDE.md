...
> [!CAUTION]
> **v0 boundaries — promise nothing undelivered.** The tracker MCP server (`src/tracker/`) is **built but not installed** (no `.mcp.json` / no `[mcp_servers]`). `backpressure index` is a **stub** (`not yet implemented`); `backpressure build` now compiles/previews the per-target config (read-only, does not install). The store is a **JSON file**. The Stop-hook command **defaults to `pnpm test`** and is configurable via `init --gate <command>` (§3.7.3). And **there is no bundled loop runner** — `Governor`, `writeJournalEntry`, and `runAgent` are **primitives you assemble yourself** (§3.11). Cross-ref §1.6, §4.5.
