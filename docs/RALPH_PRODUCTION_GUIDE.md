...
> [!CAUTION]
> **v0 boundaries — promise nothing undelivered.** The tracker MCP server (`src/tracker/`) is **built but not installed** (no `.mcp.json` / no `[mcp_servers]`). `backpressure build` previews the per-target config and `backpressure index` reports the installed capabilities (both read-only; neither installs into a repo). The store is a **JSON file**. The Stop-hook command **defaults to `pnpm test`** and is configurable via `init --gate <command>` (§3.7.3). And **there is no bundled loop runner** — `Governor`, `writeJournalEntry`, and `runAgent` are **primitives you assemble yourself** (§3.11). Cross-ref §1.6, §4.5.
